...
(async () => {
  // ---- sendIntakeForms sends both, with the right templates ----
  {
    ...
    ok("auto-send is ONLY SSA-1696 (827 needs attestation)", res.length === 1 && res[0].code === "SSA-1696");
    ok("1696 used its template + action", sent.find((s) => s.templateId === "T1696" && s.actionId === "A1696"));
    ok("827 NOT auto-sent", !sent.find((s) => s.templateId === "T827"));
    ...
    // 827 still sendable manually
    const r827 = await svc.sendForm("u", "K1", "SSA-827");
    ok("827 sends manually via sendForm", r827.code === "SSA-827" && sent.find((s) => s.templateId === "T827"));
  }
